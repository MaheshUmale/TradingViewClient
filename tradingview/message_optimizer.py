...
Implements efficient message queues, deduplication, priority handling, and batch operations.
"""
...
class ProcessedMessage:
    """A message that has been processed and enriched"""
    message_id: str
...
class MessageDeduplicator:
    """Deduplicates incoming messages to avoid redundant processing"""
    ...
    def is_duplicate(self, message: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if fingerprint in self.message_timestamps:
                logger.debug(f"Duplicate message found: {fingerprint}")
                return True
    ...
    def _generate_fingerprint(self, message: Dict[str, Any]) -> str:
        """Generates a fingerprint for a message"""
        try:
            ...
            # For data messages, include a data digest
            if 'data' in message:
            ...
            logger.error(f"Failed to generate message fingerprint: {e}")
            return str(hash(str(message)))

    def _cleanup_expired_messages(self, current_time: float) -> None:
        """Removes expired fingerprints based on TTL"""
        try:
            expired_keys = [
            ...
            logger.error(f"Failed to clean up expired messages: {e}")


class MessageClassifier:
    """Classifies raw messages into types and assigns priorities"""
    ...
    def classify_message(self, raw_message: Dict[str, Any]) -> ProcessedMessage:
        """Classifies a raw message into a ProcessedMessage object"""
        try:
    ...
    def _extract_symbol(self, message: Dict[str, Any]) -> Optional[str]:
        """Attempts to extract the trading symbol from the message"""
        try:
            # Try extracting symbol from different locations
...
class BatchProcessor:
    """Handles batching of messages to improve throughput"""
    ...
    async def add_message(self, message: ProcessedMessage) -> None:
        ...
        try:
            ...
            # Add to the corresponding type's batch
            self.pending_batches[message_type].append(message)

            # Set the timer for the first message in the batch
            if message_type not in self.batch_timers:
                self.batch_timers[message_type] = time.time()

            # Check if the batch should be processed immediately
            ...
            logger.error(f"Failed to add message for batch processing: {e}")

    async def _process_batch(self, message_type: MessageType) -> None:
        """Processes a batch of a specific message type"""
        try:
            batch = self.pending_batches[message_type]
            if not batch:
                return
    ...
    async def _handle_batch(self, message_type: MessageType, batch: List[ProcessedMessage]) -> None:
        """Handles a specific batch of messages"""
        async with self.processing_semaphore:
            try:
                start_time = time.time()

                # Group processing by symbol
                ...
                logger.debug(f"Batch processed: {message_type.name}, "
                           f"messages: {len(batch)}, took: {processing_time:.1f}ms")
    ...
    async def _process_symbol_group(self,
        ...
        """Processes a group of messages for a single symbol"""
        try:
            # Business logic for merging updates could go here
    ...
    async def _merge_kline_updates(self, symbol: str, messages: List[ProcessedMessage]) -> None:
        """Merges multiple K-line updates, keeping only the latest"""
        try:
            # Sort by timestamp
            messages.sort(key=lambda m: m.timestamp)

            # Logic: Keep the latest full update
            latest_message = messages[-1]
            latest_message.processed = True

            # Mark others as processed (discarded)
    ...
    async def _merge_quote_updates(self, symbol: str, messages: List[ProcessedMessage]) -> None:
        """Merges multiple quote updates, keeping only the latest"""
        try:
            # Keep the latest quote
            latest_message = messages[-1]
            latest_message.processed = True
    ...
    async def flush_all_batches(self) -> None:
        """Flushes all currently pending batches"""
        try:
    ...
    def get_stats(self) -> Dict[str, Any]:
        """Returns statistics for the batch processor"""
        return {
...
class AdvancedMessageOptimizer:
    """Advanced message optimizer orchestrating deduplication, classification, and batching"""
    ...
    async def add_message(self, raw_message: Dict[str, Any]) -> bool:
        """Adds a raw message to be processed"""
        try:
    ...
    async def _handle_single_message(self, message: ProcessedMessage) -> None:
        """Handles a single message directly if batching is disabled"""
        try:
            ...
            if handler:
                ...
                logger.debug(f"No handler found for message type: {message.message_type.name}")
    ...
    def get_comprehensive_stats(self) -> Dict[str, Any]:
        """Returns comprehensive statistics about the optimizer"""
        stats = {
