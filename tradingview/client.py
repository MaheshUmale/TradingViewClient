...
class Client:
    """
    Client for interacting with TradingView's WebSocket data services.

    Provides high-level methods for connecting to TradingView, managing sessions,
    and receiving real-time market data and indicators. Supports both authenticated
    and anonymous access.

    Attributes:
        is_logged (bool): Whether the client has successfully authenticated.
        is_open (bool): Whether the WebSocket connection is currently active.
        sessions (dict): Mapping of active session IDs to their handlers.
    """
    def __init__(self, options: Optional[Dict[str, Any]] = None, **kwargs):
        """
        Initializes the TradingView Client.

        Args:
            options (Optional[Dict[str, Any]]): Configuration dictionary.
                Supported keys: 'token', 'signature', 'server', 'heartbeat_interval', 'DEBUG'.
            **kwargs: Individual configuration options passed as keyword arguments.
        """
    ...
    async def connect(self) -> bool:
        """
        Establishes a WebSocket connection to the TradingView server.

        Performs the following steps:
        1. Initiates the WebSocket handshake.
        2. Configures authentication (using token or anonymous).
        3. Starts the background heartbeat and message processing tasks.

        Returns:
            bool: True if connection was successful, False otherwise.
        """
        url = f"wss://{self._server}.tradingview.com/socket.io/websocket?type=chart"
        # url = f"wss://data.tradingview.com/socket.io/websocket?auth=sessionid"
        # url =f"wss://data.tradingview.com/socket.io/websocket?from=chart%2FN8zfIJVK%2F&date=2026-06-12T09%3A00%3A32&type=chart&auth=sessionid"
        try:
            import websockets
            ...
            #             'Pragma':'no-cache' ,
            #             'Connection':'Upgrade' ,
            #             'Sec-WebSocket-Key':'X+t9AkyEDVtc5eGim7Iz2g==' ,
                ...
                try:
                    if version_parts[0] >= 6:
                        try:
                            ...
                            self._logged = False
                            if connection_closed_ok and isinstance(e, connection_closed_ok):
                                logger.info(f"Connection closed normally: {str(e)}")
                            else:
