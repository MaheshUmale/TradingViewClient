...
"""
TradingView Realtime Data Adapter
Specifically handles realtime data streams, cache management, and event distribution
"""
...
class RealtimeDataBuffer:
    ...
    def _cleanup_expired_data(self) -> None:
        ...
        try:
            ...
            # Remove expired data from the left
...
class EventDispatcher:
    ...
    def unregister_handler(self, event_type: EventType, handler: Callable) -> bool:
        ...
        try:
            ...
            logger.warning(f"Event handler not found to unregister: {event_type.name}")
            return False
    ...
    async def _dispatch_loop(self) -> None:
        ...
        while self.is_running:
            try:
                ...
                if handlers:
                    # Concurrently execute all handlers
                    tasks = []
                    for handler in handlers:
                        try:
                            if asyncio.iscoroutinefunction(handler):
                                ...
                                # Synchronous handlers execute in thread pool
                    ...
                    if tasks:
                        ...
                        for i, result in enumerate(results):
                            if isinstance(result, Exception):
                                logger.error(f"Event handler {i} execution failed: {result}")
                ...
                logger.error(f"Event dispatch loop exception: {e}")
                self.event_stats['events_failed'] += 1
    ...
    def _update_dispatch_time_stats(self, dispatch_time: float) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to update dispatch time statistics: {e}")
...
class AdvancedRealtimeAdapter:
    """Advanced realtime data adapter"""
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.is_running = True
            logger.info("✅ Advanced realtime data adapter initialized successfully")
            return True

        except Exception as e:
            logger.error(f"❌ Advanced realtime data adapter initialization failed: {e}")
            return False
    ...
    async def shutdown(self) -> None:
        ...
        try:
            ...
            logger.info("Advanced realtime data adapter closed")

        except Exception as e:
            logger.error(f"Failed to close advanced realtime data adapter: {e}")
    ...
    async def subscribe_symbol_data(self, symbol: str, subscription_type: SubscriptionType,
        ...
        try:
            ...
            # Send subscription status event
    ...
    async def unsubscribe_symbol_data(self, symbol: str, subscription_type: SubscriptionType) -> bool:
        ...
        try:
            ...
            # Send unsubscription status event
            await self.event_dispatcher.dispatch_event(RealtimeEvent(
                event_type=EventType.SUBSCRIPTION_STATUS,
                symbol=symbol,
    ...
    async def process_realtime_data(self, symbol: str, raw_data: Dict[str, Any],
        ...
        try:
            ...
            if quality_score < 0.5:  # Quality too low
            ...
            if subscription and subscription.is_active:
                try:
                    ...
                    logger.error(f"Failed to call subscription callback for {symbol}: {e}")
                    subscription.error_count += 1

                    # If too many errors, suspend subscription
                    if subscription.error_count > 10:
                        subscription.is_active = False
                        logger.warning(f"Subscription for {symbol} suspended due to excessive errors")
            ...
            logger.error(f"Failed to process realtime data for {symbol}: {e}")
            return False

    def get_symbol_buffer_data(self, symbol: str, count: int = 100) -> List[Tuple[float, Any]]:
        """Get symbol buffer data"""
        try:
            buffer = self.data_buffers.get(symbol)
            if buffer:
            ...
            logger.error(f"Failed to get buffer data for {symbol}: {e}")
            return []
        ...
        try:
            ...
            if 'time' in data:
                try:
                    ...
                    if time_diff > 3600:  # Over 1 hour
                        score *= 0.7
                    elif time_diff > 300:  # Over 5 minutes
                        score *= 0.9
...
async def test_realtime_adapter():
    ...
    try:
        ...
        async def on_kline_data(symbol: str, data: Dict[str, Any]):
            print(f"Received Kline data: {symbol} {data['close']}")

        # Subscribe to BTC 15m Kline
