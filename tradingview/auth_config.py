...
class TradingViewAuthManager:
    ...
    def get_account(self, account_name: Optional[str] = None) -> Optional[TradingViewAccount]:
        ...
        try:
            import browser_cookie3
            cookies = browser_cookie3.brave(domain_name='tradingview.com')
            if cookies:
            ...
            logger.debug(f"Failed to fetch cookies from Brave using browser_cookie3: {e}")
...
def to_cookiejar(raw_cookies):
    ...
    for cookie_data in raw_cookies:
        cookie = Cookie(
            ...
            path_specified=bool(cookie_data.get('path')),
            secure=cookie_data.get('secure', False),
            expires=cookie_data.get('expires'),
            discard=False,
            comment=None,
            comment_url=None,
            rest={'HttpOnly': str(cookie_data.get('httpOnly', False))},
            rfc2109=False
        )
...
def get_tradingview_cookie_jar(account_name: Optional[str] = None):
    ...
    try:
        ...
        raw_cookies = browser_cookie3.brave(domain_name='tradingview.com')
        if raw_cookies:
            ...
            if auth and auth['token'] == session_id:
                return to_cookiejar(raw_cookies) 
        cookies = browser_cookie3.brave(domain_name='tradingview.com')
        if cookies:
            return cookies
...
def auto_login_from_brave():
    ...
    try:
        ...
        if raw_cookies:
            ...
            # (usually longest or most recent, but browser_cookie3 doesn't give timestamps easily)
        ...
        logger.error("browser_cookie3 not installed. Cannot auto-login from Brave.")
    except Exception as e:
