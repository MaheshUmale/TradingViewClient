...
def graphic_parse(raw_graphic=None, indexes=None):
    """
    Parses raw graphic data from TradingView.

    Args:
        raw_graphic: The raw dictionary containing drawing data.
        indexes: A list of time indexes corresponding to bar offsets.

    Returns:
        dict: A dictionary containing parsed labels, lines, boxes, tables, etc.
    """
...
def get_cells(table_id, cells_data):
    """
    Retrieves and organizes table cells into a matrix.

    Args:
        table_id: The ID of the table to filter for.
        cells_data: Raw dictionary of cell data.

    Returns:
        list: A 2D list (matrix) representing the table rows and columns.
    """
    matrix = []
